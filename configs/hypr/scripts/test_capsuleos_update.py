#!/usr/bin/env python3
"""
Self-test for capsuleos-update.py against throwaway git repos in a temp dir. No
...
spec = importlib.util.spec_from_file_location("capsuleos_update", HERE / "capsuleos-update.py")
...
def main():
    ...
    origin = tmp / "origin"
    data = tmp / "data" / "capsuleos-update"
    state = tmp / "state"
    config = tmp / "config"
