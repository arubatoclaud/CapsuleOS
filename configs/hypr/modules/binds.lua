...
hl.bind("Print", hl.dsp.exec_cmd("sh -c 'f=\"$HOME/Pictures/Screenshots/$(date +%Y%m%d-%H%M%S).png\"; mkdir -p \"${f%/*}\"; g=\"$(slurp)\" || exit 0; grim -g \"$g\" \"$f\" && wl-copy --type image/png < \"$f\" && notify-send \"Screenshot\" \"Copied to clipboard — ${f##*/}\"'")) -- screenshot
