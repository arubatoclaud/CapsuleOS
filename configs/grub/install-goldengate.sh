#!/usr/bin/env bash
# Install the Goldengate GRUB theme + curated 3-entry boot menu (CachyOS New/Old, Windows).
# Run as root:  sudo bash configs/grub/install-goldengate.sh
set -euo pipefail

REPO="$(cd "$(dirname "$0")" && pwd)"
SRC="$REPO/themes/goldengate"
DEST="/boot/grub/themes/goldengate"
...
cp -n "$GRUBDEF" "$GRUBDEF.bak-pre-goldengate" || true
...
cp -f "$REPO/10_capsuleos" /etc/grub.d/10_capsuleos
chmod 755 /etc/grub.d/10_capsuleos
...
echo "==> DONE. Reboot to see goldengate theme."
