 ...
 * to the capsuleos-wallpaper-dir state file on its last run, then
 * ~/CapsuleOS/wallpapers for a first boot before wallpaper.sh init has run.
 *
...
Singleton {
    ...
    readonly property string wpDir: Flags.wallpaperDir.length > 0 ? Flags.wallpaperDir
        : (resolvedDir.length > 0 ? resolvedDir : Quickshell.env("HOME") + "/CapsuleOS/wallpapers")
    readonly property string thumbDir: (Quickshell.env("XDG_CACHE_HOME") || (Quickshell.env("HOME") + "/.cache")) + "/capsuleos-wp-thumbs/"
    ...
    readonly property string stateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/capsuleos-wallpaper"
    readonly property string dirStateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/capsuleos-wallpaper-dir"
