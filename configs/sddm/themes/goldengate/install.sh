...
sudo rm -f "${DEST_DIR}/install.sh" "${DEST_DIR}/Xsetup-goldengate.sh"

echo ":: Installing X11 setup script (primary output + cursor warp)"
sudo install -D -m 0755 "${SRC_DIR}/Xsetup-goldengate.sh" /etc/sddm/Xsetup-goldengate.sh
sudo tee "${CONF_DIR}/20-xsetup.conf" >/dev/null <<EOF
[X11]
DisplayCommand=/etc/sddm/Xsetup-goldengate.sh
EOF
